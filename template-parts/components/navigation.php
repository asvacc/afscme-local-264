<nav x-data="{menu_open: false}" @resize.window.debounce="if(window.innerWidth > 1023) menu_open = false" x-init="$watch('menu_open', value => $dispatch('menu-toggled', value))" class="relative">
    ...
    <?php wp_nav_menu([
        ...
        'items_wrap' => '<ul id="%1$s" x-cloak 
							...
							x-show="menu_open"
                            class="%2$s">%3$s</ul>',
        'menu_class' => 'fixed overflow-y-auto lg:hidden top-0 pt-20 flex h-full w-[20rem] flex-col items-stretch right-0 justify-start transition-transform bg-green-dark ',
    ]); ?>
